...
| `football.html` | **Pitch** — match history, Barça (red) vs Madrid (blue) team cards (collapsed by default, tap to show players), formation modal, then **Last match performance** (top 3 + MVP + Clowned) and monitor-only **Player performance** logging. Monitors edit teams in place (no tab in the Monitor panel) |
...
- **Pitch points (V17.6)** — `football-points.js` draws two things: `#pitchLastMatch` (public — top 3 + MVP + Clowned cards for the most recent match, tap for a stat breakdown) and `#performance` (monitor-only, the whole section stays `hidden` for everyone else — a monitor taps a player and logs stats directly, no approval step). The scoring table (`SCORING`), `scorePerformance`, and `matchBoard` (this match's top 3/MVP/clown) live in `football-data.js`; retune points THERE. Collection `pitchPerformances/{YYYY-MM-DD_playerId}`, public read, **monitor write only — republish `firestore.rules`**. A season-wide leaderboard is planned for the Rankings page, not here.
