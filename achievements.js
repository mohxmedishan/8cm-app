...
function renderPublic(){
  ...
  const groups=new Map(); cache.forEach(a=>{if(!groups.has(a.studentId))groups.set(a.studentId,[]);groups.get(a.studentId).push(a);});
  c.innerHTML=[...groups.entries()].map(([sid,items])=>`<article class="achievement-group"><h3 class="achievement-student">${escapeHtml(studentName(sid))}</h3><ul class="achievement-list">${items.map(a=>`<li class="achievement-item"><span class="task-tag announcement">${escapeHtml(a.category||"General")}</span><div class="achievement-body"><p class="task-subject">${escapeHtml(a.title)}</p>${a.description?`<p class="task-detail">${escapeHtml(a.description)}</p>`:""}${a.date?`<p class="achievement-date">${escapeHtml(a.date)}</p>`:""}</div></li>`).join("")}</ul></article>`).join("");
}
...
function renderManage(){
  ...
  list.innerHTML=""; cache.forEach(a=>{const row=document.createElement("div");row.className="manage-row";row.innerHTML=`<div class="manage-row-body"><p class="task-subject">${escapeHtml(a.title)}</p><p class="task-detail">${escapeHtml(studentName(a.studentId))} · ${escapeHtml(a.category||"General")}${a.date?" · "+escapeHtml(a.date):""}</p></div><div class="task-monitor-actions"><button class="task-icon-btn" data-action="edit" data-id="${escapeHtml(a.id)}">✎</button><button class="task-icon-btn task-icon-btn-danger" data-action="delete" data-id="${escapeHtml(a.id)}">✕</button></div>`;list.appendChild(row);});
...
function openForm(a){const f=$("achievementForm");if(!f)return;populateStudentSelect();editingId=a?.id||null;f.studentId.value=a?.studentId||f.studentId.options[0]?.value||"";f.title.value=a?.title||"";f.description.value=a?.description||"";f.category.value=a?.category||"General";f.date.value=a?.date||new Date().toISOString().slice(0,10);setFormError(null);f.hidden=false;f.querySelector('button[type="submit"]').textContent=a?"Save changes":"Add achievement";playOpen();f.title.focus();}
...
async function handleSubmit(e){e.preventDefault();const f=e.target;const payload={studentId:f.studentId.value,title:f.title.value.trim(),description:f.description.value.trim(),category:f.category.value,date:f.date.value||""};if(!payload.studentId||!payload.title)return;setFormError(null);const b=f.querySelector('button[type="submit"]');const old=b.textContent;b.disabled=true;b.textContent="Saving…";try{if(editingId){await updateDoc(doc(db,"achievements",editingId),payload);await logAction("updated",{resourceType:"achievement",resourceId:editingId,summary:`Updated achievement for ${studentName(payload.studentId)}: ${payload.title}`});}else{const ref=await addDoc(collection(db,"achievements"),{...payload,awardedAt:serverTimestamp()});await logAction("created",{resourceType:"achievement",resourceId:ref.id,summary:`Awarded ${payload.title} to ${studentName(payload.studentId)}`});}playSuccess();closeForm({silent:true});}catch(err){console.error("Save failed:",err);playError();setFormError("Couldn't save that — check your monitor access and try again.");}finally{b.disabled=false;b.textContent=old;}}
async function handleDelete(id){if(!confirm("Delete this achievement?"))return;try{await deleteDoc(doc(db,"achievements",id));await logAction("deleted",{resourceType:"achievement",resourceId:id,summary:"Deleted achievement"});playDelete();}catch(err){console.error("Delete failed:",err);playError();alert("Couldn't delete that — check your monitor access.");}}
export function initAchievements(){const needs=!!$("achievementsList")||!!$("achievementManageList");if(!needs)return;startListener();onStudents((list)=>{latestStudents=list;renderPublic();renderManage();const f=$("achievementForm");if(f&&!f.hidden)populateStudentSelect();});loadStudents().catch(()=>{});latestStudents=getStudentsSync();subscribeAuth(({monitor})=>{isCurrentMonitor=monitor;const b=$("addAchievementBtn");if(b)b.hidden=!monitor;renderManage();});const add=$("addAchievementBtn");if(add)add.addEventListener("click",()=>openForm(null));const f=$("achievementForm");if(f){f.addEventListener("submit",handleSubmit);const c=f.querySelector('[data-action="cancel"]');if(c)c.addEventListener("click",()=>closeForm());}}
