...
export const changelog = [
  {
    ...
    version: "16.1",
    items: [
      "Fixed arrange mode: the two-dash handle now sits to the left of each row's title, drag works with mouse and touch, and arrow keys reorder a focused handle. The pencil button only appears once a section has more than one item, and only for monitors.",
      "Arrange mode is live: a new homework, announcement, or resource added while arranging appears immediately at the top of its group without kicking you out of arrange mode.",
      ...
      "Dragging an Important item below a Normal one demotes it on save. Dragging a Normal item to the top keeps it Normal and demotes the Important items beneath it. Demotion only takes effect when saving.",
      "Pinned Announcements and Resources stay in their own block above everything else. You can reorder within the block, but you cannot drag an item out of it.",
      "While arranging, Pin/Edit/Delete controls are hidden and homework filters are disabled so nothing fires accidentally.",
      "A toggle with no drags writes nothing. A save failure leaves arrange mode active with the new order intact so it can be retried.",
      "Fixed the Announcements Pin button: it was writing to the legacy announcements collection instead of the live notices collection, so it silently failed.",
    ],
  },
