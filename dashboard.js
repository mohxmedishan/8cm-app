// ============================================
// 8CM — "My Day" personal dashboard
// ------------------------------------------------
// Only renders once a user is signed in AND has claimed a student
// identity — before that, index.html's Homework/Announcements panels
// underneath still work for anyone, same as they always were public.
// This module adds no Firestore reads of its own: it composes data
// that assignments.js / announcements.js / events.js / timetable-data.js
// are already fetching and caching for their own panels.
// ============================================
...
function renderClassStatus() {
  ...
  if (status.next) {
    parts.push(`<span><strong>Next:</strong> ${status.next.full}</span>`);
  }
...
function renderHeader() {
  ...
  stats.innerHTML = `
    <span class="profile-stat-pill">Roll #${latestStudent.rollNumber}</span>
    <span class="profile-stat-pill house-${latestStudent.house}"><span class="house-dot ${latestStudent.house}"></span>${houseLabel(latestStudent.house)}</span>
  `;
}

export function initDashboard() {
  if (!$("myDayCard")) return; // not the homepage

  subscribeAuth(({ profile }) => {
    latestStudent = profile && profile.claimedStudentId ? students.find((s) => s.id === profile.claimedStudentId) : null;
    renderHeader();
  });
